...
public class CameraSimulator implements ISimulatorUpdater
{
    ...
    {
        // mLoopsBetweenUpdates = (int) Math.ceil((1.0 / sFRAMES_PER_SECOND) /
        // HAL.getCycleTime());
        // mLoopsStale = (int) Math.ceil((sLATENCY_MS * 1e-3) /
        // HAL.getCycleTime());

        mLoopsBetweenUpdates = (int) Math.ceil((1.0 / sFRAMES_PER_SECOND) / .02);
        mLoopsStale = (int) Math.ceil((sLATENCY_MS * 1e-3) / .02);
...
//            System.out.println("Ditching " + aPeg.mName + " because of Angle" + aTargetInfo.mAngle + "[" + sMAX_VIEWABLE_ANGLE + "]");
    ...
    @Override
    public void setRobot(IRobotClassContainer aContainer)
    {
        RobotBase robot = ((JavaRobotContainer) aContainer).getJavaRobot();
        mPositioner = ((Snobot2017) robot).getPositioner();
    }
